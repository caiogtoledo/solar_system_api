...
#         resp = self.dynamo.get_item(partition_key=self.partition_key_format(
#             user_id), sort_key=self.sort_key_format(user_id))
...
#         resp = self.dynamo.delete_item(partition_key=self.partition_key_format(
#             user_id), sort_key=self.sort_key_format(user_id))
...
#         resp = self.dynamo.update_item(partition_key=self.partition_key_format(
#             user_id), sort_key=self.sort_key_format(user_id), update_dict=item_to_update)
...
#         counter = int(self.dynamo.get_item(
#             partition_key='COUNTER', sort_key='COUNTER')['Item']['COUNTER'])
#         print(f"counter: {counter}")
#         resp = self.dynamo.update_item(partition_key='COUNTER', sort_key='COUNTER', update_dict={
#                                        'COUNTER': Decimal(counter+1)})
#         print(f"resp: {resp}")
